...
let surrounding_occupied_seats (line_index : int) (row_index : int) (map : position_map) : int =
  let line_size = map_line_size map in
  let row_size = map_row_size map in
  let surrounding_seats = [
    (line_index - 1, row_index - 1);
    (line_index - 1, row_index);
    (line_index - 1, row_index + 1);
    (line_index, row_index - 1);
    (line_index, row_index + 1);
    (line_index + 1, row_index - 1);
    (line_index + 1, row_index);
    (line_index + 1, row_index + 1)] in
  let filtered_surround_seats = List.filter (fun (line_index, row_index) -> (line_index >= 0) && (line_index < line_size) && (row_index >= 0) && (row_index < row_size)) surrounding_seats in
  let surrounding_occupied_seats = List.filter (fun (line_index, row_index) -> (seat_status line_index row_index map) = '#') filtered_surround_seats in
  List.length surrounding_occupied_seats
;;
...
let transform_at_position (line_index : int) (row_index : int) (map : position_map) : char =
  ...
  | '.' -> '.'
  | '#' -> (match (surrounding_occupied_seats line_index row_index map) >= 4 with | true -> 'L' | false -> '#')
  | 'L' -> (match (surrounding_occupied_seats line_index row_index map) > 0 with | true -> 'L' | false -> '#')
