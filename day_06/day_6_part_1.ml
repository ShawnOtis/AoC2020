...
let yes_answers_per_group (group_answers : group_answers) : answers =
  List.sort_uniq (Char.compare) (List.flatten group_answers)
;;
...
let highest_ticket_from_file (filename : string) : int =
  ...
  let answers_in_group = List.map yes_answers_per_group group_answers in
